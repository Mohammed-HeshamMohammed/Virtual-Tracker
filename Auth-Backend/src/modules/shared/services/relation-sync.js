...
import { deactivationGovernanceForRole } from "../../../core/middleware/auth/role-hierarchy.js";
import { validateOwnerRoleChange } from "../../../core/middleware/auth/role-owner-policy.js";
...
export async function syncMemberPrimaryRole(db, memberId, roleName, assignedBy = "", actorRoleName = "") {
  ...
  const { invalidateMemberRoleCache } = await import("../../../core/middleware/auth/role-cache.js");
...
export async function enrichTeamProjectsWithNames(db, rows) {
  ...
  for (let i = 0; i < projectIds.length; i += 30) {
    ...
    const refs = chunk.map((id) => db.collection("projects").doc(id));
    const snaps = await db.getAll(...refs);
