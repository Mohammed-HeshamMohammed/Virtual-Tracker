...
import { getAuthAdmin, getDb } from "../../core/database/firebase.js";
import { getAuthContext, requireManagementRole } from "../../core/middleware/auth/auth-context.js";
import { canViewerManageInvite } from "../../core/middleware/auth/invite-scope.js";
import { readJsonBody } from "../../core/middleware/http/read-json-body.js";
import { sendJson } from "../../core/middleware/http/response.js";
import { normalizeDoc } from "../shared/services/schema-normalize.js";
import { USER_PROFILES_COLLECTION } from "../auth/profile/profile-collection-name.js";
import { upsertProfileFromUserRecord } from "../auth/profile/profile-sync.js";
import { sendPreprovisionWelcomeEmail } from "../auth/email/preprovision-email.js";
import { getEmailDeliveryConfig } from "../auth/email/email-config.js";
import { assertEmailCanUseMemberInviteOrPreprovision, validateEmailsForAddMembersFlow } from "../shared/services/eligibility.js";
import { recordMemberRelationship } from "../shared/services/member-relationships.js";
import { isExcludedFromHierarchy } from "../shared/services/hierarchy-placement.js";
import { validateMemberNamePart } from "../shared/services/member-display-name.js";
import { assertValidPhone } from "../../core/middleware/http/validate-body.js";
import { resolveMemberRoleName } from "../shared/services/activity-scope.js";
import {
...
} from "../shared/services/relation-sync.js";
import { validateRegistrationPassword } from "../../core/middleware/auth/password-validation.js";
import { logSafeError, logSafeWarn } from "../../core/middleware/http/sanitize-error.js";
import { validateRoleAssignment } from "../../core/middleware/auth/role-assignment-guard.js";
import {
  checkInviteRegisterAllowed,
  recordInviteRegisterFailure,
  recordInviteRegisterSuccess,
} from "../../core/middleware/security/invite-abuse-guard.js";
import { resolveAppPublicUrl } from "../auth/flow/app-public-url.js";
import {
  assertInviteAvailableForRegistration,
  resolveInviteExpiryMs,
  shareLinkInviteFields,
} from "../shared/services/invite-lifecycle.js";
import {
  getInviteManagementLink,
  renewInviteForManagement,
  resendInviteEmailForManagement,
} from "../shared/services/invite-management.service.js";
 ...
 */
export async function routeInvites(req, res, url, origin) {
  const db = getDb();
  if (!db) return false;
  ...
  if (pn.startsWith("/api/public/invites/") && pn.endsWith("/register") && req.method === "POST") {
    ...
    try {
      ...
      const { upsertMemberPayRate } = await import("./member-profile.service.js");
      ...
      if (inviteKind === "email") {
        try {
          const { maybeSendRegistrationWelcomeEmail } = await import("../auth/email/registration-welcome-email.js");
