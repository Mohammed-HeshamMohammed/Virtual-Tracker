...
const MODAL_LABEL = "mb-1.5 block text-[10px] font-bold uppercase tracking-wider text-slate-400"
const MODAL_INPUT =
...
const DROPDOWN_TRIGGER_CLASS =
  "flex w-full items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-left text-sm text-slate-700 transition-colors hover:border-blue-400"
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className={cn(DROPDOWN_TRIGGER_CLASS, open && "border-blue-400 ring-1 ring-blue-400/20")}
      >
      ...
      {open &&
        ...
        createPortal(
          <>
            ...
            <div
              className={cn(
                "fixed flex max-h-72 flex-col overflow-hidden rounded-xl border border-slate-100 bg-white py-1 shadow-lg",
                ...
                <div className="border-b border-slate-100 p-2">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Search"
                      className="w-full rounded-md border border-slate-200 py-1.5 pl-8 pr-2 text-xs focus:border-blue-400 focus:outline-none focus:ring-1 focus:ring-blue-400/20" aria-label="Interactive control"
                    />
                  </div>
                  <button
                    type="button"
                    ...
                    className="mt-2 w-full py-1.5 text-left text-xs font-semibold text-blue-600 hover:underline"
                  >
                    Edit options
                  </button>
                </div>
                ...
                {loading ? (
                  ...
                  filtered.map((opt) => (
                    <button
                      ...
                      className={cn(DROPDOWN_OPTION_CLASS, opt === value && "bg-blue-50 font-medium text-blue-800")}
                    >
                      {opt}
                    </button>
                  ))
          ...
          >
            ...
            >
              <div className="mb-4 flex items-start justify-between gap-3">
                <div>
                  <h2 className="text-lg font-bold text-slate-800">New option</h2>
                  <p className="mt-1 text-sm text-slate-500">This will update selection choices in all member profiles.</p>
                </div>
                <button type="button" onClick={() => !saving && setNewOpen(false)} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="mb-1.5 text-xs text-slate-500">{rankHelpText}</p>
                  <select
                    value={rankBand}
