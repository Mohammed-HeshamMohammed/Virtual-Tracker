...
type Envelope = { success?: boolean; error?: string; data?: GeneralDashboardPayload }

let cached: { at: number; data: GeneralDashboardPayload } | null = null
const CACHE_TTL_MS = 60_000

export function invalidateGeneralDashboardCache(): void {
  cached = null
}

export async function fetchGeneralDashboardData(options?: {
  force?: boolean
  signal?: AbortSignal
}): Promise<GeneralDashboardPayload> {
  ...
  const data = await retryWithBackoff(
    async () => {
      const res = await apiFetch(`${API_BASE}/api/dashboard/general`, {
        headers: { Accept: "application/json" },
        signal: options?.signal,
      })
      const json = await readJsonSafe<Envelope>(res)
      if (!res.ok) throw extractApiError(res.status, "Failed to load dashboard", json)
      if (!json?.success || !json.data) throw new Error(json?.error || "Failed to load dashboard")
      return json.data
    },
    {
      ...
      onRetry: (attempt, delayMs, error) => {
        logSafeWarn(`[general-dashboard] fetch retry #${attempt} in ${delayMs}ms`, error)
      },
    },
  )
