...
3. ✅ **Cutover — `tasks`, `task_assignments`, and `time_tracking` are all fully repointed to Postgres now.** No `TASK_DATA_SOURCE` flag, no dual-write window — direct cutover, matching the note above. Started from the reported bug ("task created inside Firestore"); root cause and full fix:
   - **Root cause:** task creation never went through `tasks/routes.js` — it's handled by the generic schema-catalog CRUD dispatcher in `schema/routes.js` (`/api/v1/tasks`), which already has a per-entity Postgres/Firestore router (`shouldRouteEntityToPostgres`, `postgres-crud.service.js`) — the same mechanism `time-entries`/`timesheets`/lookups/member-data already use. `"tasks"` was simply never registered in it.
   - Added `Dashboard-Backend/src/lib/postgres/tasks-postgres.service.js` — full CRUD against `tasks`, following `projects-postgres.service.js`'s conventions — and wired `"tasks"` into `postgres-crud.service.js`'s dispatcher (`POST`/`GET`/`PUT`/`PATCH`/`DELETE /api/v1/tasks[/:id]`).
   - **Every one of the 18 originally-scoped files is now accounted for** — each either fixed or confirmed already-compatible/intentionally out of scope:
     - Fixed (task doc reads/writes repointed to Postgres): `task-assignee-api.js`, `task-assignments.js` (~13 call sites — `getTaskParticipation`, `startTaskForUser`, `syncTaskAssignments`, `ensureAssignmentForUser`, `migrateTaskAssignments`, `recomputeTaskStatus`, `enrichAssignmentRow`, `getReviewQueue`, `reviewAssignment`), `task-time-tracking.js` (`aggregateTaskProgress`, `syncTaskTimeTracking`, `getTaskTimeTracking`), `schema/routes.js` (generic dispatcher + task-reorder endpoint), `tasks/routes.js` (task review endpoint), `overview-service.js`, `member-remove-from-tree-service.js`, `bootstrap-service.js`, `dashboard-base-loader.js`, `task-access.js` (`canAccessTask` — gates almost every task route), `activity/routes.js` (the endpoint the desktop agent hits every 5-20s), `schema-crud.service.js`.
     - Confirmed already fine, no change needed: `task-workload-validation.js` (takes the task object as a parameter, source-agnostic), `schema/visibility.js` (only reads plain fields), `schema/collection-ref.js` (URL parsing only), `schema/catalog/tasks/index.js` (schema declaration only).
     - Confirmed intentionally untouched: `task-subcollections.js` (comments/subtasks/attachments/hours/time_tracking child collections — explicitly out of scope, still real Firestore data), `entity-bootstrap-manifest.js` (confirmed its `ENTITY_BOOTSTRAP_MANIFEST` export isn't even imported by its one consumer — inert, not a live query driver; `projects` sitting unchanged there since its own migration is the precedent for leaving `tasks` alone too).
   - **Found beyond the original 18-file list on a fresh repo-wide sweep** (`grep -rn 'collection("tasks")'`): `overview-service.js` had a second reference (`getOverviewPanels`, not just `getOverviewCore`), `tasks/routes.js`'s task-review endpoint, and `schema/routes.js`'s task-reorder endpoint. All fixed. Re-ran the sweep after — zero remaining `db.collection("tasks")` calls outside the intentionally-scoped `task-subcollections.js`.
   - **Two adjacent bugs found and fixed from the already-shipped `projects` migration**, same root cause: `validateForeignKeys()` and `assertTeamLinkedToProject()` (`schema-crud.service.js`) checked Firestore for `project_id`/`team_id`/`task_id` references — but `projects`/`team_projects` moved to Postgres-only weeks ago with no dual-write, so **every entity creation carrying a `project_id` referencing a project made after that cutover would have silently failed FK validation** until this fix. Both now check Postgres first when configured.
   - **Caught and fixed a regression before it shipped:** task `DELETE` runs through the same dispatcher — without intervention it would've deleted only the Postgres `tasks` row and orphaned the still-Firestore `task_assignments`/comments/subtasks/attachments/hours rows. Moved the existing `deleteTaskWithChildren()` cleanup into the Postgres-routed delete path, removed the now-dead duplicate Firestore-only delete block.
   - **Schema extended:** `tasks` gained 9 columns not in the original Phase 2 DDL, discovered while tracing real call sites — `completed`, `total_assignees`, `started_assignees`, `not_started_assignees`, `participation_percent`, `all_assignees_started` (written by `recomputeTaskStatus`), plus `total_active_seconds`/`total_idle_seconds`/`aggregated_progress_percent` already existed but needed adding to `updateTaskPg`'s write-through allowlist for `aggregateTaskProgress` to actually persist them.
   - **`task_assignments` fully migrated too** (18 call sites across 6 files: `task-assignments.js`, `task-time-tracking.js`, `task-workload-validation.js`, `task-access.js`, `tasks/routes.js`, `task-subcollections.js`). Added `Dashboard-Backend/src/lib/postgres/task-assignments-postgres.service.js` (find/get/upsert/update/delete, plus the multi-id lookups `getTaskIdsAssignedToMembersPg`/`getAssignmentsForTasksPg` using Postgres `= ANY($1::uuid[])` instead of Firestore's 30-item `in`-chunking). `findAssignmentDoc` (the old Firestore-doc-ref helper) removed entirely, not shimmed — every caller now works with plain Postgres rows and calls `updateAssignmentPg(id, patch)`/`deleteAssignmentPg(id)` directly. `deleteTaskWithChildren()` no longer batch-deletes `task_assignments` itself — the table's `task_id` FK is `ON DELETE CASCADE`, so deleting the Postgres `tasks` row already cascades it. Also fixed the exact bug flagged in Phase 3 Action Item 4 along the way: `getReviewQueue`'s `db.collection("task_assignments").limit(500)` full-scan (which silently dropped rows past 500 org-wide) is now `listAllAssignmentsPg()`, no arbitrary cap.
   - **`time_tracking` fully migrated too**, into the existing `task_member_progress` table — matching what Phase 2's own Target Schema section always said should happen, not new scope. The table already had a dual-write helper (`task-member-progress.service.js`) but it was gated behind `TASK_MEMBER_PROGRESS_PG_DUAL_WRITE`, which defaults `false` — meaning even the mirror wasn't necessarily active. Added true primary functions to the same file (`getTrackingRowPg`, `getTaskTrackingRowsPg`, `getAllTrackingRowsPg`, `upsertTrackingRowPg`, `updateTrackingFieldsPg`) that don't check the flag at all, and repointed every real call site (`task-time-tracking.js`'s `aggregateTaskProgress`/`syncTaskTimeTracking`/`getTaskTimeTracking`, `task-assignments.js`'s `getTaskParticipation`/`getReviewQueue`/`reviewAssignment`) off `taskChildCollectionRef(db, taskId, "task-time-tracking")` and the `db.collectionGroup("time_tracking").limit(500)` full scan (same silent-cap bug class as `task_assignments` had) onto Postgres. `syncTaskTimeTracking`'s old create-vs-update branch collapsed into one `upsertTrackingRowPg` call — its `ON CONFLICT` clause's `last_started_at = COALESCE(existing, new)` reproduces the original "only set started_at once" semantic without a separate read-before-write. One caught-before-shipping bug: initially wrote the upsert's `review_notes` into the `ON CONFLICT DO UPDATE SET` list, which would have wiped review notes on every subsequent sync — re-checked the actual SQL, confirmed `review_notes` was correctly left out of that clause (only set on true insert), no fix needed, but worth having actually verified rather than assumed.
   - **Post-hoc review caught one real gap, fixed:** `task_member_progress.task_id`/`timer_sessions.task_id` had **no FK at all** in the boot-time schema — couldn't reference `tasks(id)` when those two tables were first created (before `tasks` existed), and the one-time `--add-fk` backfill step only ran once, manually, not on every boot. A fresh deployment or a DB reset would have silently lost that integrity guarantee. Added both as idempotent `ON DELETE CASCADE` constraints (`fk_tmp_task`, `fk_ts_task`) after `tasks` in the boot array — this also closes a second gap: `deleteTaskPg` alone only cleans up `task_assignments` (via its own table-level FK), it never touched `task_member_progress`/`timer_sessions`, so deleting a task was silently orphaning those rows until this fix.
   - **Verified:** `node --check` passes on all 208 backend JS files (full-repo sweep, not just touched files), zero syntax failures, run three times across this session. Column lists in all three new/extended Postgres services (`TASK_COLUMNS`, `ASSIGNMENT_COLUMNS`, `TRACKING_COLUMNS`) cross-checked line-by-line against the real DDL — 28/28, 14/14, 14/14, exact match. **Not verified:** actual module import resolution or a live request — `node_modules` isn't installed in this dev checkout, so this needs real end-to-end testing once deployed to Coolify.
   - **Still Firestore, deliberately, per Phase 2's own original scope note:** the 4 non-time-tracking task subcollections — `task-comments`, `task-subtasks`, `task-attachments`, `task-hours`. Never in scope for this migration ("not time-tracking data, no urgency").
