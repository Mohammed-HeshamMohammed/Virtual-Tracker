-- Virtual Tracker — PostgreSQL schema reference (every table, index,
-- function, and structural ALTER applied at runtime).
--
...
--
-- Deliberately omitted: triggers, views, and one-time data-fix statements
-- (role renames, legacy FK backfills, seed rows). Those replay history a
-- fresh database never had; this file documents structure, not the boot log.
