...
GROUP BY task_id`,
  ...
  // A row per start, pause, resume and stop, and per refused dashboard
  // request (D3). No foreign key: history must outlive a deleted session.
  ...
  `UPDATE activity_sessions
     SET status = 'stopped', ended_at = now(), updated_at = now(),
         stop_reason = COALESCE(stop_reason, 'duplicate_close')
  ...
  `ALTER TABLE projects ADD COLUMN IF NOT EXISTS break_time_seconds INTEGER NOT NULL DEFAULT 600`,
  `ALTER TABLE projects ADD COLUMN IF NOT EXISTS disable_break_limit BOOLEAN NOT NULL DEFAULT false`,
  `UPDATE projects SET break_time_seconds = 600 WHERE break_time_seconds <= 0`,
  `ALTER TABLE projects DROP CONSTRAINT IF EXISTS projects_break_time_seconds_check`,
  `ALTER TABLE projects ADD CONSTRAINT projects_break_time_seconds_check CHECK (break_time_seconds > 0)`,
