...
export type PlatformOption = "windows" | "mac" | "linux"
...
export function AgentDownloadChoices({
  ...
  compact = false,
  apiBaseUrl,
}: AgentDownloadChoicesProps) {
  const options = [
    {
      id: "windows" as PlatformOption,
      title: "Windows",
      subtitle: ".exe / .msi installer",
      icon: WindowsIcon,
      badge: "Win 10/11",
    ...
    {
      id: "mac" as PlatformOption,
      title: "macOS",
      subtitle: ".dmg installer",
      icon: AppleIcon,
      badge: "macOS 11+",
      accent: "hover:border-slate-400/40 text-slate-200",
      btnClass: "bg-slate-100 hover:bg-white text-slate-900",
    },
    {
      id: "linux" as PlatformOption,
      title: "Linux",
      subtitle: ".AppImage / .deb",
      icon: LinuxIcon,
      badge: "Ubuntu / Debian",
      accent: "hover:border-amber-500/40 text-amber-400",
      ...
      <div className={`flex flex-wrap items-center gap-2.5 ${className}`}>
        {options.map((opt) => {
          ...
          return (
            <a
              key={opt.id}
              href={resolveDownloadUrl(opt.id, apiBaseUrl)}
              className={`inline-flex items-center gap-2 px-3.5 py-2 rounded-xl border border-slate-700 bg-slate-800/80 hover:bg-slate-800 text-slate-200 text-xs font-semibold transition-all ${opt.accent}`}
            >
    ...
    <div className={`grid grid-cols-1 sm:grid-cols-3 gap-4 ${className}`}>
      {options.map((opt) => {
        ...
        return (
          <div
            key={opt.id}
            className={`group relative flex flex-col justify-between p-5 rounded-2xl border border-slate-800 bg-slate-900/90 transition-all duration-200 hover:scale-[1.02] ${opt.accent}`}
          >
            <div>
              ...
              <h3 className="text-base font-bold text-white mb-0.5">{opt.title}</h3>
              <p className="text-xs text-slate-400 mb-4">{opt.subtitle}</p>
            </div>

            <a
              href={resolveDownloadUrl(opt.id, apiBaseUrl)}
              className={`w-full py-2.5 px-3 rounded-xl font-semibold text-xs text-center flex items-center justify-center gap-1.5 shadow-md transition-all ${opt.btnClass}`}
            >
