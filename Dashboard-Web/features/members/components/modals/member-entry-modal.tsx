  ...
  return (
    ...
    >
      ...
      >
        <div className="shrink-0 border-b border-slate-100 dark:border-slate-800 px-6 py-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm font-medium text-slate-700 dark:text-slate-200">{member.name}</p>
            </div>
            <button
              type="button"
              onClick={onClose}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
          ...
          {focusedAction === "reset-password" && (
            <div className="space-y-3">
              ...
              {resetMessage ? <p className="text-sm text-emerald-600 dark:text-emerald-400">{resetMessage}</p> : null}
            </div>
          )}
        ...
        <div className="flex shrink-0 justify-end gap-2 border-t border-slate-100 dark:border-slate-800 px-6 py-4">
          <button
            type="button"
            onClick={onClose}
            disabled={busy}
