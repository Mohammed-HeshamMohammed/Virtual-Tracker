...
export function NumberInput({ value, onChange, isDark }: { value: string; onChange: (v: string) => void; isDark: boolean }) {
...
export function UnitBadge({ label, isDark }: { label: string; isDark: boolean }) {
  return (
...
function SearchInput({ placeholder, value, onChange, isDark }: { placeholder: string; value: string; onChange: (v: string) => void; isDark: boolean }) {
  return (
    <div className={cn("flex items-center gap-2 px-3 py-2 border rounded-full text-sm", isDark ? "bg-[#1a2235] border-white/10 text-white" : "bg-white border-slate-200 text-slate-600")}>
      <Search className="w-3.5 h-3.5 shrink-0 opacity-40" aria-label="Interactive control" />
