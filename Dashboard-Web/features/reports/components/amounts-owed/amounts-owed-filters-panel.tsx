...
export function AmountsOwedFiltersPanel({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "z-110 flex h-[500px] w-[400px] min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm",
        className ?? "fixed right-16 top-1/2 -translate-y-1/2"
      )}
    >
      <div className="flex shrink-0 items-end justify-between gap-2 border-b border-slate-200 px-2 pt-3">
        <div className="flex min-w-0 flex-1">
          <button
            ...
            className={cn(
              ...
              tab === "filters" ? "border-blue-500 text-blue-500" : "border-transparent text-slate-400"
            )}
          >
            Filters
          </button>
          <button
            ...
            className={cn(
              "flex-1 border-b-2 pb-2.5 text-center text-[10px] font-bold uppercase tracking-wider transition-colors",
              tab === "saved" ? "border-blue-500 text-blue-500" : "border-transparent text-slate-400"
            )}
          >
            Saved filters
          </button>
        ...
        <button
          type="button"
          onClick={onClose}
          className="mb-1 shrink-0 rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-slate-100"
          aria-label="Close"
        >
          <X className="h-5 w-5" />
        </button>
      ...
      {tab === "filters" ? (
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-5 py-4 scrollbar-hide">
            <div className="space-y-5">
              ...
              <div className="relative">
                <div className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">Members</div>
                <button
                  type="button"
                ...
                <AnimatePresence>
                  {membersOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -4 }}
                      className="absolute left-0 right-0 top-full z-20 mt-1 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"
                    >
                      <div className="p-3">
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full rounded-full border border-blue-500 py-2 pl-9 pr-9 text-sm text-slate-800 outline-none placeholder:text-slate-400" aria-label="Interactive control"
                          />
                          {memberSearch ? (
                            <button
                              type="button"
                      ...
                      <div className="max-h-48 overflow-y-auto border-t border-slate-100 scrollbar-hide">
                      ...
                      <div className="border-t border-slate-100 px-4 py-3">
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            showOnlySelectedMembers ? "bg-blue-500" : "bg-slate-200"
                          )}
                        >
                          <span
                            className={cn(
        ...
        <div className="flex min-h-[280px] flex-col px-5 py-4">
          <h3 className="text-base font-semibold text-slate-800">Saved filters</h3>
          <div className="relative mt-4">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-label="Interactive control" />
            <input
              type="search"
              placeholder="Search saved filters"
              className="w-full rounded-full border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm text-slate-800 outline-none placeholder:text-slate-400 focus:border-blue-400 focus:ring-1 focus:ring-blue-400"
            />
          </div>
          <p className="mt-8 text-center text-sm text-slate-500">No saved filters</p>
        </div>
      )}
    </motion.div>
  )
