...
function downloadDailyTotalsCsv(groups: AmountsOwedDayGroup[]): void {
...
function downloadDailyTotalsPdf(groups: AmountsOwedDayGroup[], dateLabel: string): void {
  ...
  downloadReportPdf({
    ...
    filename: "daily-totals",
  })
}

/** Total amount per day, plotted from the report's own rows. */
function DailyTotalsPerDayChart({ groups }: { groups: AmountsOwedDayGroup[] }) {
  ...
  return (
    <div className="relative isolate min-h-0">
      <div className="report-print-area relative mx-auto min-h-[min(80vh,56rem)] max-w-[1400px] space-y-6 px-6 py-6">
        <ReportPageHeading title="Daily totals report" pageId="reports-daily" />
            ...
            <div className="flex flex-wrap items-stretch overflow-visible rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-[#151b2d] shadow-sm">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="min-w-36">
                  <DropdownMenuItem className="cursor-pointer" onClick={() => downloadDailyTotalsCsv(groups)}>To CSV</DropdownMenuItem>
                  <DropdownMenuItem className="cursor-pointer" onClick={() => downloadDailyTotalsPdf(groups, dateLabel)}>To PDF</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
        ...
        {chartVisible ? <DailyTotalsPerDayChart groups={groups} /> : null}
